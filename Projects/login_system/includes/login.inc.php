...
if (isset($_POST['login-submit'])) {
  ...
  // We don't have to close the MySQLi connection since it is done automatically, but it
  // is a good habit to do so anyways since this will immediately return resources to PHP
  // and MySQL, which can improve performance.
  require 'dbh.inc.php';
  ...
  // stop the rest of the script from running, and take the user back to the login form with an error message.
  ...
  if (empty($mailuid) || empty($password)) {
    ...
    // Next we need to get the password from the user in the database that has the same username
    // as what the user typed in, and then we need to de-hash it and check if it matches the password
    // the user typed into the login form.

    // We will connect to the database using prepared statements which work by us sending SQL to the
    // database first, and then later we fill in the placeholders by sending the users data.
